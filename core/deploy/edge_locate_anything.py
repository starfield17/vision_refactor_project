"""Edge deploy pipeline backed by local NVIDIA LocateAnything."""
...
from core.deploy.edge_common import FpsLimiter, append_stats_snapshot, iter_source_frames
from core.infer.locate_anything import LocateAnythingInferencer
from core.transport.stats_http import push_stats_event
from common.types.errors import DataValidationError, TransportError
...
def _save_annotated(result: Any, target_path: Path) -> bool:
    ...
    try:
        ...
        return bool(cv2.imwrite(str(target_path), result.plot()))
    except Exception:
        return False


def run_edge_locate_anything_deploy(cfg: dict[str, Any], run_ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    inferencer = LocateAnythingInferencer.from_config(cfg, confidence=confidence)
    ...
    for packet in iter_source_frames(edge_cfg):
        ...
        event = StatsEvent.now(
            ...
            run_id=run_id,
            model_id=f"locate_anything:{inferencer.model_id}",
            backend="locate_anything",
            transport_mode="edge-locate-anything",
        )
        ...
        if endpoint:
            try:
                ...
                stats_failed += 1
                logger.warn(
                    "deploy.edge.locate_anything.stats_push.failed",
        ...
        logger.info(
            "deploy.edge.locate_anything.frame",
            "Frame processed with LocateAnything",
            run_id=run_id,
            source_id=source_id,
            frame_index=packet.frame_index,
