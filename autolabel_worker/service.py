...
from autolabel_worker.config.schema import load_config


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Autolabel worker API service")
...
def create_app(role_cfg: dict[str, Any], config_path: Path, workdir_override: str | None = None):
    try:
        ...
        raise ConfigError(f"fastapi is required for autolabel worker API: {exc}") from exc
    ...
    runner = SubprocessJobRunner(
        job_store=store,
        log_dir=Path(role_cfg["workspace"]["root"]) / "tmp" / "jobs" / "autolabel_worker",
        worker_module="autolabel_worker.worker",
    )
    app = FastAPI(title="Vision Autolabel Worker API", version="1.0.0")
    ...
    def health() -> dict[str, Any]:
        return {"ok": True, "service": "autolabel_worker", "job_db_path": str(store.db_path)}
    ...
    def status(authorization: str | None = Header(default=None)):
        ...
        return {
            "ok": True,
            "role": "autolabel_worker",
            "service": "autolabel_worker",
    ...
    def submit_autolabel(payload: dict[str, Any], authorization: str | None = Header(default=None)):
        ...
        overrides.extend(build_autolabel_overrides_from_payload(payload))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        validate_service_security("autolabel_worker", cfg["server"])
        start_control_plane_heartbeat(
            cfg,
            role="autolabel_worker",
            service="autolabel_worker",
