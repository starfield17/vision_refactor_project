"""Deploy edge CLI entrypoint (Phase 5)."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Phase 5 deploy edge CLI")
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="KEY=VALUE",
        help="Config override, repeatable. Example: --set deploy.edge.source=images",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("deploy.edge.cli.start", "CLI started", config_path=str(config_path))
