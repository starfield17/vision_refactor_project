...
def resolve_path_fields(cfg: dict[str, Any], base_dir: Path) -> dict[str, Any]:
    return resolve_config_paths(cfg, base_dir, PATH_FIELDS)
...
def _resolve_config_base_dir(config_path: Path) -> Path:
    # For work-dir config snapshots, keep relative paths anchored at the repo root.
...
def load_toml_config(
    ...
    """Load a role-specific TOML config."""
    if not config_path.exists():
        raise ConfigError(f"Config file not found: {config_path}")
    ...
    if workdir_override:
        merged["workspace"]["root"] = workdir_override
    if path_fields:
        merged = resolve_config_paths(merged, _resolve_config_base_dir(config_path), path_fields)
