...
from autolabel.config.schema import load_config as load_autolabel
...
from train.config.schema import load_config as load_train


class DistributedRoleConfigTests(unittest.TestCase):
    def test_role_example_configs_load(self) -> None:
        cases = [
            ("train", load_train, Path("train/config/config.example.toml")),
            ("autolabel", load_autolabel, Path("autolabel/config/config.example.toml")),
    ...
    def test_role_config_adapts_to_kernel_shape(self) -> None:
        cfg = load_train(Path("train/config/config.example.toml").resolve())
        kernel_cfg = role_to_kernel_config(cfg, "train", "train")
    ...
    def test_control_plane_store_tracks_jobs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ControlPlaneStore(Path(tmp) / "control.db")
            job = store.create_job(kind="edge_run", target_role="edge", payload={"max_frames": 1})
            self.assertEqual(job["status"], "queued")
            attached = store.attach_upstream(
                job["job_id"],
                target_node_id="edge-001",
                upstream_endpoint="http://127.0.0.1:7813",
                upstream_job_id="upstream-1",
                status="running",
            )
            self.assertIsNotNone(attached)
            self.assertEqual(attached["upstream_job_id"], "upstream-1")
            self.assertEqual(store.list_jobs()[0]["target_role"], "edge")
